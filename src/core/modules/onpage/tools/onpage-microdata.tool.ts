...
export class OnPageMicrodataTool extends BaseTool {
  ...
  getDescription(): string {
    return 'Get structured data/microdata from OnPage crawl. Returns information about Schema.org markup, Open Graph tags, and other structured data found on pages. Essential for rich snippets optimization and social media sharing.';
  }
  ...
  async handle(params: any): Promise<any> {
    try {
      ...
      const response = await this.dataForSEOClient.makeRequest(
        `/v3/on_page/microdata/${params.id}`,
        'POST',
        [payload]
      );
