...
export class OnPageDuplicateContentTool extends BaseTool {
  ...
  getDescription(): string {
    return 'Get pages with duplicate content. Identifies pages with similar or identical content which can harm SEO rankings. Helps prioritize content optimization and canonical tag implementation.';
  }
  ...
  async handle(params: any): Promise<any> {
    try {
      ...
      const response = await this.dataForSEOClient.makeRequest(
        `/v3/on_page/duplicate_content/${params.id}`,
        'POST',
        [payload]
      );
